# FLEET-vm-designer.md — vm-designer (worker pane, fleet `hackhouse-dev`)
...
You are the **worker** pane of the `hackhouse-dev` fleet, tmux target `hackhouse-dev:vm-designer`.
...
## North star
Design ONE small, concrete, genuinely diverse VM concept (not another ML-detector or security-scanner clone -- something like a language-runtime playground, a data-format converter bench, a puzzle/game engine, or similar). Write the design brief to a shared file (DESIGN-BRIEF.md in this cwd) for vm-builder2 to build from: purpose, base image, packages, 2-3 concrete usage examples that prove it works. Do NOT build it yourself -- design only, then stop.
...
## Done = (measurable)
- **Design ONE small, concrete, genuinely diverse VM concept (not another ML-detector or security-scanner clone -- something like a language-runtime playground, a data-format converter bench, a puzzle/game engine, or similar). Write the design brief to a shared file (DESIGN-BRIEF.md in this cwd) for vm-builder2 to build from: purpose, base image, packages, 2-3 concrete usage examples that prove it works. Do NOT build it yourself -- design only, then stop.** is actually true, verified by something you ran/observed this session — not merely attempted.
...
- Read `./FLEET-vm-designer.md` (this file) and `./CLAUDE.md` if present, then take the smallest concrete action toward: **Design ONE small, concrete, genuinely diverse VM concept (not another ML-detector or security-scanner clone -- something like a language-runtime playground, a data-format converter bench, a puzzle/game engine, or similar). Write the design brief to a shared file (DESIGN-BRIEF.md in this cwd) for vm-builder2 to build from: purpose, base image, packages, 2-3 concrete usage examples that prove it works. Do NOT build it yourself -- design only, then stop.**

## Rails / autonomy envelope
- Do not reach outside this pane's own cwd/target.
- Escalate to the overseer (via OVERSEER-STATUS-vm-designer.md, not direct pane injection) instead of self-expanding scope.
...
1. **Check for concurrent occupants first.** Run
   ...
   write until you've noted the overlap in `OVERSEER-STATUS-vm-designer.md` and, if real changes are
...
3. **If real edits are actually part of your task and this cwd is a git repo, isolate
   ...
   --show-current`; you should be on `agent/hackhouse-dev-vm-designer` already, and a
   ...
   add ../$(basename "$(git rev-parse --show-toplevel)")-worktrees/hackhouse-dev-vm-designer
   -b agent/hackhouse-dev-vm-designer` (idempotent: check `git worktree list` first if you
...
4. **Never merge, rebase onto, or push into the operator's own branch — ever, on your own
   initiative.** Commit your own work to your own `agent/hackhouse-dev-vm-designer` branch in
   ...
   it prints `AUDIT: PASS` should `OVERSEER-STATUS-vm-designer.md` say the branch is ready for human
...
- This pane: `hackhouse-dev:vm-designer` (role: worker)
- Peers: vm-designer,vm-builder2

## Reporting
When you stop (finish a turn), append a ≤5-line summary (what changed / next /
blockers) to `./OVERSEER-STATUS-vm-designer.md` in this cwd. **APPEND — if that file already has
content, read it first and add your entry below it; never `Write` the whole file and
replace what's there.** (If `OVERSEER-STATUS-vm-designer.md` isn't the plain `OVERSEER-STATUS.md`, it's
