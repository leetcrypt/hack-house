# FLEET-vm-builder2.md — vm-builder2 (worker pane, fleet `hackhouse-dev`)
...
You are the **worker** pane of the `hackhouse-dev` fleet, tmux target `hackhouse-dev:vm-builder2`.
...
## North star
Wait for DESIGN-BRIEF.md from vm-designer (poll for it, do not proceed without it), then build EXACTLY that VM via the hh-operator skill doctrine: host/join a room, drive the sandbox, install the real software, verify it works. Save + publish it. Then -- this is the untested half -- RELOAD it: pull/load the saved VM back (fresh sandbox or via /sbx pull) and verify it still works post-reload, not just that it saved. CRITICAL: do NOT tear down your room, sandbox, or tmux session when done -- leave everything running exactly as-is for manual human review. Report clearly in OVERSEER-STATUS-vm-builder2.md whether reload succeeded, with evidence.
...
## Done = (measurable)
- **Wait for DESIGN-BRIEF.md from vm-designer (poll for it, do not proceed without it), then build EXACTLY that VM via the hh-operator skill doctrine: host/join a room, drive the sandbox, install the real software, verify it works. Save + publish it. Then -- this is the untested half -- RELOAD it: pull/load the saved VM back (fresh sandbox or via /sbx pull) and verify it still works post-reload, not just that it saved. CRITICAL: do NOT tear down your room, sandbox, or tmux session when done -- leave everything running exactly as-is for manual human review. Report clearly in OVERSEER-STATUS-vm-builder2.md whether reload succeeded, with evidence.** is actually true, verified by something you ran/observed this session — not merely attempted.
...
- Read `./FLEET-vm-builder2.md` (this file) and `./CLAUDE.md` if present, then take the smallest concrete action toward: **Wait for DESIGN-BRIEF.md from vm-designer (poll for it, do not proceed without it), then build EXACTLY that VM via the hh-operator skill doctrine: host/join a room, drive the sandbox, install the real software, verify it works. Save + publish it. Then -- this is the untested half -- RELOAD it: pull/load the saved VM back (fresh sandbox or via /sbx pull) and verify it still works post-reload, not just that it saved. CRITICAL: do NOT tear down your room, sandbox, or tmux session when done -- leave everything running exactly as-is for manual human review. Report clearly in OVERSEER-STATUS-vm-builder2.md whether reload succeeded, with evidence.**

## Rails / autonomy envelope
- Do not reach outside this pane's own cwd/target.
- Escalate to the overseer (via OVERSEER-STATUS-vm-builder2.md, not direct pane injection) instead of self-expanding scope.
...
1. **Check for concurrent occupants first.** Run
   ...
   write until you've noted the overlap in `OVERSEER-STATUS-vm-builder2.md` and, if real changes are
...
3. **If real edits are actually part of your task and this cwd is a git repo, isolate
   ...
   --show-current`; you should be on `agent/hackhouse-dev-vm-builder2` already, and a
   ...
   add ../$(basename "$(git rev-parse --show-toplevel)")-worktrees/hackhouse-dev-vm-builder2
   -b agent/hackhouse-dev-vm-builder2` (idempotent: check `git worktree list` first if you
...
4. **Never merge, rebase onto, or push into the operator's own branch — ever, on your own
   initiative.** Commit your own work to your own `agent/hackhouse-dev-vm-builder2` branch in
   ...
   it prints `AUDIT: PASS` should `OVERSEER-STATUS-vm-builder2.md` say the branch is ready for human
...
- This pane: `hackhouse-dev:vm-builder2` (role: worker)
- Peers: vm-designer,vm-builder2

## Reporting
When you stop (finish a turn), append a ≤5-line summary (what changed / next /
blockers) to `./OVERSEER-STATUS-vm-builder2.md` in this cwd. **APPEND — if that file already has
content, read it first and add your entry below it; never `Write` the whole file and
replace what's there.** (If `OVERSEER-STATUS-vm-builder2.md` isn't the plain `OVERSEER-STATUS.md`, it's
