...
| _(interim)_ | Browser access for mobile — a *second*, detached onion service fronting `web-relay` (its own unmerged branch), `--public-url` pointed at it so `/web link`/the share file print the onion address. Guest needs Orbot/Tor Browser (a stock browser can't open `.onion` at all — inherent, not a bug). Optional add-on to terminal hosting, not a replacement — both onions share one tor daemon/ControlPort. `hh-go onion browser` automates the sequence (personal deployment script, not part of this repo). Live-verified 2026-09-01: found (during testing) a 21-hour-old orphaned relay silently squatting the relay port — `/health` answered fine but wasn't the process just started, exactly the pitfall the project's own `hh-howto.sh` already documents. Fixed with a PID-match guard (recorded pid vs. `health['pid']`); reproduced the collision on purpose afterward to confirm it now refuses cleanly instead of proceeding against the wrong process. Both onions then reachable via an independently-bootstrapped guest client. | — |
...
3. **Bundled vs. system `tor`.** v1 assumes a system-installed `tor` binary
   (already true on a typical Linux desktop). Bundling a static Tor binary is a later
