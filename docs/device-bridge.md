...
- **`~/pineapple-pager-c2/`** — staged C2 (01-connect → 02-payloads →
  ...
  `~/.ssh/<pager-key>`). **`payloads/user/remote_access/tg_command_bot/tg_shim.sh`** is
...
- **`~/flipper-0/`** — Flipper C2 (serial `/dev/ttyACM*`, firmware/apps/
...
3. **Bridge confinement** — the bridge execs *only* allowlisted adapter argv (never a host
   shell); a room member cannot pivot device-verb → host-command. Runs least-priv.
...
- **Auth:** key `~/.ssh/<pager-key>` (`IdentitiesOnly`) — NOT the `sshpass`/`gopass`
